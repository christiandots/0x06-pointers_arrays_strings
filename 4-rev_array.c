 ...
 if(rows%2!=0) 
{
 m=0;
 n=rows-1;
 //Taking input for Magic Square printf("Enter Magic Square Numbers Horizontally\n");
 //If 8 1 6 // 3 5 7 // 4 9 2 //Then Enter from 8 to 6 and then 3 to 7 and then 4 to 9 for(i=0;i<rows;i++) 
{
 for(j=0;j<col;j++) 
{
 scanf("%d",&a[i][j]);
 ...
 for(j=0;j<col;j++) 
{
 ...
 //Finding Sum for 2 Diagonals for(i=0;i<rows;i++) 
{
 diagonal_1+=a[i][i];
 diagonal_2+=a[m][n];
 m++;
 ...
 else 
{
 ...
 //Finding Sum of Rows for(i=0;i<rows;i++) 
{
 sum_rows=0;
 for(j=0;j<col;j++) 
{
 ...
 else 
{
 ...
 if(flag==1) printf("Given Matrix is Magic Square\n");
